<div class="card-title">
    ...
    <b><?php echo translate('short_list')?></b></h3>
</div>

<div class="card-body">
    <?php if($this->db->get_where("member", array("member_id" => $this->session->userdata('member_id')))->row()->is_closed == 'yes'){?>
        <div class="col-md-12">
            <p class="c-base-1 pt-4 text-center">"<?php echo translate('your_account_is_closed!_please_re-open_the_account_to_see_your_short_list!')?>"
            </p>
...
<script>
    ...
    function confirm_interest(id) {
        ...
        if (isloggedin == "") {
            ...
            if (rem_interests <= 0) {
                ...
                $("#modal_body").html("<p class='text-center'><b><?php echo translate('remaining_express_interest(s): ')?>"+rem_interests+" <?php echo translate('times')?></b><br><?php echo translate('you_have_no_express_interests_left. please_buy_any_package_from_premium_plans.')?></p>");
    ...
    function confirm_remove(id) {
        ...
        if (isloggedin == "") {
            ...
            $("#active_modal").modal("toggle");
            $("#modal_header").html("<?=translate('confirm_remove')?>");
            $("#modal_body").html("<p class='text-center'>Are you sure that you want to Remove this Member from Shortlist?</p>");
            $("#modal_buttons").html("<button type='button' class='btn btn-danger btn-sm btn-shadow' data-dismiss='modal' style='width:25%'><?php echo translate('close')?></button> <a href='#' id='confirm_remove' class='btn btn-sm btn-base-1 btn-shadow' onclick='return remove_shortlist("+id+")' style='width:25%'><?php echo translate('confirm')?></a>");
        }    
        return false;
    }
            ...
            setTimeout(function() {
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        $(".ajax_danger_alert").html("<?php echo translate('you_have_removed_this_member_from_shortlist!')?>");
                        $('#ajax_success_alert').fadeOut('fast');
                        setTimeout(function() {
                            $('#ajax_danger_alert').fadeOut('fast');
                        }, 5000); // <-- time in milliseconds
...
<script>
    ...
    function filter_my_interets(page){      
        ...
        var url = '<?php echo base_url(); ?>home/ajax_short_list/'+page;
        var place = $('#result');
        var formdata = false;
        if (window.FormData){

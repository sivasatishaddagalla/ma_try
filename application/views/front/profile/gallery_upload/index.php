...
<script>
    ...
    var ispackagevalid = "<?=$this->Crud_model->package_validation_check()?>";
    ...
    function confirm_gallery_upload(id) {
        ...
        else if(ispackagevalid != 1) {
            $("#active_modal").modal("toggle");
            $("#modal_header").html("<?php echo translate('package_expired')?>");
            $("#modal_body").html("<p class='text-center'><?php echo translate('your_package_has_been_expired.')?></p>");
            $("#modal_buttons").html("<button type='button' class='btn btn-danger btn-sm btn-shadow' data-dismiss='modal' style='width:25%'><?=translate('close')?></button> <a href='<?=base_url()?>home/plans' class='btn btn-sm btn-base-1 btn-shadow' style='width:30%'><?php echo translate('purchase_package')?></a>");
        }
        else {
            if (rem_photos <= 0) {
                $("#active_modal").modal("toggle");

                                    ...
                                    <tr>
                                        ...
                                        <td>
                    		            	...
                    		            	if (file_exists('uploads/profile_image/'.$image[0]['thumb'])) {?>
												<a  href="#">
                                                	<img src="<?=base_url()?>uploads/profile_image/<?=$image[0]['thumb']?>" class='img-sm'>
												</a>
											<?php }?>
                                        </td>
                                        <td>
                                        ...
                                        <td>
                                            <button  data-toggle='modal' data-target='#profile_image_status_modal' class='btn btn-success btn-xs add-tooltip' data-toggle='tooltip' data-placement='top' title='<?php echo translate('approve'); ?>' onclick="profile_image_status(<?=$member->member_id?>, 1)">
                                                <i class='fa fa-thumbs-up'></i>
                                            </button>
											<?php if ($member->profile_image_status != '2') {?>
                                                <button  data-toggle='modal' data-target='#profile_image_status_modal' class='btn btn-danger btn-xs add-tooltip' data-toggle='tooltip' data-placement='top' title='<?php echo translate('reject'); ?>' onclick="profile_image_status(<?=$member->member_id?>, 2)">
...
<script type="text/javascript">
	...
	$("#image_status").click(function(){
    	$.ajax({
		    url: "<?=base_url()?>admin/member_profile_image_approval/change_status/"+$("#image_status").val()+"/"+$("#member_id").val(),
		    success: function(response) {
				window.location.href = "<?=base_url()?>admin/member_profile_image_approval";
		    },
			fail: function (error) {
			    alert(error);
			}

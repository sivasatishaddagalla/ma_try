...
class Crud_model extends CI_Model
{
    ...
    function package_validation_check(){
        if ($this->session->userdata('member_id') != '') {
            $package_invalid_at = $this->db->get_where('member', array('member_id' => $this->session->userdata('member_id')))->row()->package_invalid_at;
            if($package_invalid_at > date('Y-m-d')){
                return 1;
            }
            else{
                return 0;

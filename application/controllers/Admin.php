...
class Admin extends CI_Controller {
	...
	function members($para1="",$para2="",$para3="",$para4="")
	{
		if ($this->admin_permission() == FALSE) {
				...
				elseif ($para2=="do_add") {
		            ...
		            if ($this->form_validation->run() == FALSE) {
	                    ...
	                    // --------------------------------- Additional Personal Details--------------------------------- //
							...
							$data['package_invalid_at'] = date('Y-m-d', strtotime($this->db->get_where('plan', array('plan_id'=> $plan))->row()->duration .'days'));
			...
			elseif ($para1=="upgrade_member_package") {
        		...
        		$prev_duration = $this->db->get_where('member', array('member_id' => $up_member_id))->row()->package_invalid_at;
		        ...
		        $data['package_invalid_at'] 	= date('Y-m-d', strtotime( $prev_duration. ' +'. $this->db->get_where('plan', array('plan_id' => $plan_id))->row()->duration .'days'));
	...
	function member_restore($para1)
	{
		...
		$data['package_invalid_at'] = $this->db->get_where("deleted_member", array("member_id" => $para1))->row()->package_invalid_at;
	...
	function member_delete($para1){
		...
		$data['package_invalid_at'] = $this->Crud_model->get_type_name_by_id('member', $para1, 'package_invalid_at');
	...
	function earnings($para1="",$para2="")
	{
		if ($this->admin_permission() == FALSE) {
			...
			elseif ($para1 == "accept_payment") {
                ...
                $data['package_invalid_at'] = date('Y-m-d', strtotime( $member_details->package_invalid_at. '+'. $plan_details->duration .'days'));

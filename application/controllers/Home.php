...
class Home extends CI_Controller {
    ...
    function listing($para1="",$para2="")
    {
        ...
        elseif ($para1=="home_search") {
            ...
            $page_data['search_member_type'] = "all";
            $page_data['page_url'] = "home/listing/home_search";
            recache();
            $this->load->view('front/index', $page_data);
        }
        ...
        if ($this->input->post('payment_type') == 'paypal') {
            ...
            if($this->input->post('stripeToken')) {
                ...
                if($charge->paid == true) {
                    ...
                    $prev_package_invalid_at = $this->db->get_where('member', array('member_id' => $payment->member_id))->row()->package_invalid_at;
                    ...
                    $data2['package_invalid_at']    = date('Y-m-d', strtotime( $prev_package_invalid_at. '+'. $this->db->get_where('plan', array('plan_id' => $payment->plan_id))->row()->duration .'days'));
  ...
  function instamojo_success(){
     try
     {
       ...
       $instamojo_account_type = $this->Crud_model->get_settings_value('business_settings', 'instamojo_account_type', 'value');
          ...
          if(isset($response['payments'][0]['status']) )
          {
            ...
            $data1['package_invalid_at']    = date('Y-m-d', strtotime( $prev_package_invalid_at. '+'. $this->db->get_where('plan', array('plan_id' => $payment->plan_id))->row()->duration .'days'));
                    ...
                    if ($safe == 'yes') {
                        ...
                        if ($this->Crud_model->get_settings_value('third_party_settings', 'captcha_status', 'value') == 'ok') {
                            ...
                            if ($response['success']) {
                                ...
                                $data['package_invalid_at'] 	= date('Y-m-d', strtotime($this->db->get_where('plan', array('plan_id'=> 1))->row()->duration .'days'));
    ...
    function package_expiry_check(){
        $members = $this->db->get('member')->result_array();
        foreach($members as $member){
            $package_expire_date = $member['package_invalid_at'];
            if($package_expire_date < date('Y-m-d')){

                $this->db->where('member_id', $member['member_id']);
                $this->db->update('member', array('package_invalid_at' => null));
